...
│   ├── index.css               # Global styles (if any – currently minimal or empty) <br>
│   ├── components/ <br>
...
where the standard deviation σ is controlled by the energy level:
- High energy → small σ → tight, focused jet.
- Low energy → large σ → wide, flickering base (onion-like shape).
...
|--------------------|----------------------------------------------|-----------------------------------------------------|
| **Initial upward velocity** | Simulates hot gas rising                     | Stronger at higher energy (`2.0 + energy² × 10.0`)   |
| **Buoyancy (constant upward acceleration)** | Sustains rise as particles cool              | `+ (0.02 + energy × 0.1) × Δt × 60`                 |
| **Turbulence**     | Creates swirling, chaotic motion              | Low-frequency sine/cosine noise based on position and time |
| **Air drag**       | Slows particles over time                    | Velocity multiplied by `(1 - (0.3 + energy × 0.4) × Δt)` |
...
- Global energy level (higher energy → longer life).
- Distance from center (center particles live longer → dense core).
...
- Hottest regions (base/center) → white/yellow.
- Cooler regions (top/edges) → orange → red → dark.

The color is computed per particle based on **life fraction** (remaining life / initial life):

1. **>90% life**: White → bright yellow (inner core).
2. **70–90%**: Yellow → orange.
3. **40–70%**: Orange → red.
4. **<40%**: Red → dark red (dying embers).
...
- **Vertex Colors**: Each particle has its own color → smooth gradients without textures.
...
The global "energy" slider (10–100%) does not scale everything linearly. Most parameters use **quadratic ramping** (`energy²`) to feel more natural:
...
By combining:
- Gaussian spawning → natural density,
- Simple physics forces → turbulent motion,
- Lifetime/color gradients → temperature realism,
- Additive rendering + bloom → visual glow,
