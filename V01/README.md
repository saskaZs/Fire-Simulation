# 🔥 2D Fire Simulation with React and Canvas

A realistic, interactive 2D fire simulation built with React and native HTML Canvas. This educational project demonstrates a particle system for rendering dynamic fire effects in a browser, with physics, color transitions, and a simple glow effect—all without external 3D libraries.
...
- **Interactive Controls:** Energy slider (10–100%) for real-time adjustments.
...
This clusters particles centrally for a bright core, with sparser edges—mimicking real flame density.

### 3. Physics and Forces

Particles evolve under simple forces, applied per frame (Δt ≈ 1/60s):

| Force              | Purpose                                      | Implementation Details                              |
|--------------------|----------------------------------------------|-----------------------------------------------------|
| **Initial upward velocity** | Simulates rising hot gas                     | Negative Y (up); stronger at high energy (`-1.5 - energy × 4.5`) |
| **Buoyancy (acceleration)** | Sustains ascent as particles cool            | Constant upward add: `- (0.02 + energy × 0.1) × Δt` |
| **Turbulence**     | Adds swirling chaos                           | Sine/cos noise: `Math.sin(frame + pos) × amplitude` |
| **Air drag**       | Decelerates over time                        | Velocity × `(1 - dragCoeff × Δt)`; higher drag at low energy |
...
Slider (10–100%) uses quadratic scaling (`energy²`) for intuitive feel:
- Spawn rate: 6–15 particles/frame.
- Forces/lifetimes ramp non-linearly—small increases yield big visual changes, like real fire.

### 7. Performance Considerations

- Fixed canvas size (800x600) for consistency.
- Particle cap implicit via spawn/decay balance.
- No GPU shaders—pure 2D context for accessibility.
